...
% \end{equation*}
%\end{satz}
%Mit anderen Worten: $\lambda$ ist ein Eigenwert von $A$ genau dann wenn $R(\lambda)$
%ein Eigenwert von $R(A)$ ist.
%
%Dabei ist jetzt $R(\lambda)$ die formale rationale Funktion $R$ angewandt auf die komplexe Zahl $\lambda$.
%
%\begin{definition}
% Für ein gegebenes Einschrittverfahren heißt die dazugehörige Funktion $R : \C \to \C$
% \emph{Stabilitätsfunktion} des Verfahrens.
%\end{definition}
%
%\bigskip
%
...
% \text{für alle Eigenwerte $\lambda$ von $A$}
%\end{equation*}
%(plus Zusatzbedingungen für den Fall $\Re \lambda = 0$).
%
%
%
%\bigskip
...
%\end{defi}
%\begin{bsp}
%Explizites Euler-Verfahren: \begin{equation*}
%	x_{k+1}=\underbrace{\left(I+\tau A \right)}_{R \left(\tau A \right)}x_k \implies R(z)=1+z
%\end{equation*}
...
%\medskip
%
%Im Falle der skalaren Gleichung mit $\R \ni \lambda<0$ und dem Euler-Verfahren führt das auf
%die Bedingung $\tau \leq \frac{2}{\abs{\lambda}}$.
%
%\medskip
%
%Für eine graphische Darstellung der Stabilitätsgebiete expliziter Runge-Kutta-Verfahren
%siehe \cite[Seite~238]{deuflhard_bornemann:2008}. \\
%
...
%\begin{defi}
%Ein Einschrittverfahren heißt A-stabil, falls sein Stabilitätsgebiet die negative komplexe Halbebene enthält.
%\end{defi}
...
%
%\begin{lemma}
%Die Flüsse aller expliziten Runge-Kutta-Verfahren (für lineare Gleichungen) sind Polynome in $\tau A$,
%also $\Psi^{\tau} x=P (\tau A)x$.
%\end{lemma}
%\begin{proof}
% Wir zeigen dass für jedes $i\le s$ der Ausdruck $\tau k_i$ ein formales Polynom in $\tau A$ ist.
%
...
%\end{proof}
%Implizite Verfahren können A-stabil sein: z.B. Implizites Euler-Verfahren: \begin{equation*}
%	R(z)=\frac{1}{1-z}
%\end{equation*}
%Stabilitätsgebiet: $S=\{ z \in \C \mid \abs{1-z} \geq 1 \} \supset \C$.
%
%\medskip
%
...
%	Unter einem \emph{allgemeinen Runge-Kutta-Verfahren} (kurz: RK-Verfahren) verstehen wir ein Verfahren der Form
%	\begin{align*}
%		(i) && k_i &= f\Big(t + c_i \tau, x + \tau \sum_{j=1}^s a_{ij} k_j\Big), \qquad i=1,\dots,s, \\
%		(ii) && \Psi^{t + \tau,t} x &= x + \tau \sum_{j=1}^s b_j k_j. 
%	\end{align*}
%\end{defi}
%\begin{itemize}
%	\item Unterschied zu expliziten RK-Verfahren: Die Summe in \emph{(i)} geht über alle $s$ Stufen, nicht nur über die ersten $i-1$.
%\end{itemize}
...
%
%\begin{itemize}
%	\item Verfahren ist explizit, wenn $\mathcal A$ eine strikte untere Dreiecksmatrix ist.
%	\item Ansonsten muss in jedem Schritt ein nichtlineares Gleichungssystem gelöst werden.
%\end{itemize}
%\textbf{Frage:} Unter welchen Bedingungen ist dieses Gleichungssystem eindeutig lösbar (für hinreichend kleine $\tau$?) Nur dann kann ja von einem wohldefinierten Zeitintegrationsverfahren gesprochen werden.
%
%\medskip
%
%Um diese Frage zu klären schreiben wir das Verfahren zunächst in der sogenannten
%\emph{symmetrischen Form} auf:
%
%Definiere:
%\begin{equation*}
...
%	Die Abbildung $f \in C(\Omega, \R^d)$ sei auf $\Omega \subset \R \times \R^d$ bezüglich $x$ lokal Lipschitz-stetig. Für ein implizites RK-Verfahren gibt es zu jedem $(x,t) \in \Omega$ ein $\tau_\ast > 0$, und \emph{eindeutige} stetige Funktionen
%	$$ g_i \; : \; (-\tau_\ast, \tau_\ast) \to \R^d, \qquad i=1,\dots,s,$$
%	so dass:
%	\begin{enumerate}
...
% Es seien $E_1$ und $E_2$ zwei Banach-Räume.  $U$ und $V$ seien offene Kugeln in $E_1$
% (bzw.\ $E_2$) jeweils um $0$; der Radius von $V$ sei $\beta$.
...
% eine eindeutige Abbildung $g : U \to V$ so dass
% \begin{equation*}
%  g(\tau) = F(\tau,g(\tau))
% \end{equation*}
...
%
%
%\begin{proof}[Beweis von Satz~\ref{eq:wellposedness_of_implicit_rk_methods}]
%	\begin{itemize}
%		\item Sei $(t_0,x_0) \in \Omega$ fest gewählt.
%		\item $f$ ist lokal Lipschitz-stetig bezüglich $x$. D.h. es gibt Parameter $\tau_1, \rho, L > 0$ 	 so dass
%		$$ \abs{f(t,x) - f(t,\bar x)} < L \abs{x - \bar x} $$
...
%		%
%		\item Schreibe das RK-System als parameterabhängige Fixpunktgleichung
%		$$ g(\tau) = F(\tau,g(\tau)) $$
%		mit 
%		\begin{align*}
...
%		\item Damit ist 
%		$$ F: U \times V \rightarrow \R^{s\cdot d} $$\
...
%\begin{itemize}
%	\item Alles etwas komplizierter: Es müssen mehr Koeffizienten bestimmt werden.
%	\item Alles etwas einfacher: Für eine gegebene Ordnung $p$ erhält man die gleiche Anzahl von Bestimmungsgleichungen wie im expliziten Fall \cite[Satz~4.24]{deuflhard_bornemann:2008}. Man hat aber mehr Freiheitsgrade, um diese zu erfüllen.
%\end{itemize}
%
%\begin{satz}
...
%
%\begin{itemize}
%	\item Wie für explizite Verfahren zeigt man: Ein implizites Verfahren ist genau dann invariant
%	unter Autonomisierung, wenn es konsistent ist, und
...
%\begin{itemize}
%	\item Dafür braucht man die Stabilitätsfunktion.
%\end{itemize}
%
%\begin{lemma} 
...
%		\item Lösung: $x(\tau) = e^{\tau}$, RK-Approximation $\Psi^\tau$.
...
%	Warum?
%	\begin{itemize}
%		\item Angenommen es gäbe Polynome $P, Q$ mit
%			$$ \deg P \le k, \qquad \deg Q \le j,$$
%			und $k+j < p$.
%		\item Das hieße $P(z) / Q(z) - e^z = \mathcal O(z^{k+j+2})$.
%		\item Multiplikation mit $Q(z)$: $P(z) - Q(z)e^z =  \mathcal O(z^{k+j+2})$.
%		\item Daraus folgt $P = Q = 0$. Widerspruch!
%	\end{itemize}
%	Beweis davon: Übung. \cite[Lemma~6.4]{deuflhard_bornemann:2008}
%	\todoannot{1.2\baselineskip}{Hier ausarbeiten!}
%\end{proof}
%
%
%
...
%\begin{itemize}
%	\item Die folgende Idee ist unabhängig von den RK-Verfahren entwickelt worden.
%	\item Dass man dadurch implizite RK-Verfahren erhält wurde erst in den 1970ern entdeckt.
%\end{itemize}
%Betrachte 
%$$ x' = f(t,x) $$
%\begin{itemize}
...
%				$$ \Psi^{t+\tau,t} x \colonequals u(t+\tau). $$ \todoannot{\baselineskip}{Ein Bild!}
%		\end{enumerate}
%		Diese Bedingungen nennen wir \emph{Kollokationsbedingungen}.
%\end{itemize}
%Einziger Parameter des Verfahrens: Die Stützstellen $c_1, \dots, c_s$. Wir haben $s+1 $ Bedingungen an ein Polynom $s$-ten Grades.
%\begin{itemize}
%	\item Wir \emph{vermuten}, dass ein eindeutiges $u$ existiert (zumindest für kleine $\tau$).
%	\item Klar ist das nicht, denn die Gleichungen für $u$ sind nichtlinear!
%\end{itemize}
...
%		$$ k_i = f\Big(t + c_i \tau, x + \tau  \sum_{j=1}^{s} a_{ij} k_j \Big), \qquad i=1,\dots,s.$$
...
%  \begin{equation*}
%   k_i = u'(t+c_i\tau),\quad i=1,\hdots,s.
%  \end{equation*}
%
...
%\bigskip
%
%Aber sind alle Kollokations-Verfahren auch \emph{gute} RK-Verfahren?
%
%\bigskip
%
%\emph{Erinnerung:} Satz 4.18 aus \cite{deuflhard_bornemann:2008}: Ein RK-Verfahren besitzt
%\begin{itemize}
%	\item genau dann die Konsistenzordnung 1, wenn
%	\begin{equation*}
%	   \sum_{i=1}^s b_i = 1
%	\end{equation*}
%	\item genau dann die Konsistenzordnung 2, wenn zusätzlich
%	\begin{equation*}
%		\sum_{i=1}^s b_ic_i = \frac{1}{2}.
%	\end{equation*}
%\end{itemize}
%
%\begin{lemma}[Dd\ \&\ Bo, 6.37]
...
%\begin{proof}
%	\begin{itemize}
%		\item Nach Definition der $b_j$ gilt
%		\begin{equation*}
%			\sum_{j=1}^s b_jc_j^{k-1} = \sum_{j=1}^s\int_0^1 c_j^{k-1}L_j(\theta)\,d\theta.
%		\end{equation*}
%		\item $ \sum_{j=1}^sc_j^{k-1}L_j(\theta)$ ist gerade die Lagrange-Darstellung des Polynoms $\theta^{k-1}$. Deshalb
%		\begin{equation*}
...
%\end{itemize}
%
%\begin{satz}[Dd\ \&\ Bo, 6.40] \label{thm:ordnung_quadratur_kollokation}
%	Ein durch Kollokation erzeugtes implizites RK-Verfahren $(b,c,\mathcal{A})$ besitzt
%	die Konsistenzordnung $p$ für rechte Seiten $f\in\mathcal{C}^p (\Omega,\R^d)$ genau dann,
%	wenn die durch Stützstellen $c$ und Gewichte $b$ gegebene Quadraturformel die Ordnung $p$ besitzt.
%\end{satz}
%\begin{proof}[Beweisskizze] Teil 1) RK-Verfahren hat Ordnung $p$ \quad $\Rightarrow$ \quad Quadraturformel hat Ordnung $p$
%\begin{itemize}
%	\item $(b,c,\mathcal{A})$ besitze Konsistenzordnung $p$ $\Rightarrow$ Der Fehler bei einem Schritt ist $\mathcal{O}(\tau^{p+1})$.
%	\item Insbesondere können wir Gleichungen der Form $x'=f(t)$ integrieren.
%	\item Lösung davon ist $x(t+\tau) = x(t) + \int_t^{t+\tau} f(s)\,ds$.
...
%\begin{itemize}
%  \item  (jetzt wieder allgemeine $x' = f(t,x)$)
%	\item Sei $\tau $ so klein, dass das Kollokationspolynom $u\in P_s$ existiert.
%	\item Betrachte $u$ als Lösung einer Störung des AWPs
%	\begin{equation*}
...
%	\end{equation*}
%\end{itemize}
%\emph{Plan:} Schließe aus der Größe von $\delta f$ auf den Fehler $x(t+\tau) - u(t+\tau)$.
%  Das ist aber gerade der Konsistenzfehler $\Psi^{t+\tau,t} x - \Phi^{t+\tau,t} x$!
%
%\bigskip
%
%Ideen dabei: \begin{itemize}
...
%
%\begin{satz}[Aleksejew, Gröber (Satz 3.4 in \cite{deuflhard_bornemann:2008})]
...
%\end{satz}
%\begin{itemize}
%	\item Schätze das Integral mit der Quadraturformel ab
%	\begin{equation*}
%	   x(t+\tau) - u(t+\tau) = \tau \sum_{j=1}^s b_j M(t+\tau,t+c_j\tau)\,\delta f(t+c_j\tau) + \mathcal{O}(\tau^{p+1})
%	\end{equation*}
%	\item $u$ ist aber Kollokationspolynom. Deshalb ist
%	\begin{equation*}
...
%\begin{itemize}
%	\item Die Konstante in $\mathcal{O}(\tau^{p+1})$ hängt von höheren Ableitungen von $M(t+\tau,s)\delta f(s)$ nach $s$ ab.
%	\item Dieser Ausdruck hängt aber von $u$ ab.
%	\item Und $u$ wiederum hängt von $\tau$ ab!
%\end{itemize}
%Es geht aber trotzdem alles gut:\\
%(Lemma 6.41 in {{\cite{deuflhard_bornemann:2008}}}, ca. 2 Seiten lang)
%
%\bigskip
%Was haben wir gelernt?
%\begin{itemize}
%	\item In jedem Kollokationsverfahren steckt eine Quadraturformel der Ordnung $s\leq p\leq2s$.
%	\item Diese Ordnung wird an des Kollokations-Verfahren vererbt.
%	\item Das betrifft nur den Fehler am Ende eines Zeitschritts, d.h.,
%	 \begin{equation*}
%	  x(t+\tau) - u(t+\tau) = \mathcal{O}(\tau^{p+1})
%	 \end{equation*}
%	\item Gleichzeitig hat man in Form von $u$ auch eine Approximation für $x(\sigma)$
%	 für alle $\sigma$ \emph{zwischen} $t$ und $t+\tau$.
%	\item Für die gilt: $\max_{t\leq\sigma\leq t+\tau} \abs{x(\sigma)-u(\sigma)} = \mathcal{O}(\tau^{s+1})$
%	  (\cite{deuflhard_bornemann:2008}, Lemma~6.41)
%	\item Also schlechter als am Intervallende (da $s\leq p$)
%	\item Diesen Effekt nennt man Superkonvergenz.
%\end{itemize}
%
...
%\end{itemize}
%Das Optimum für $s$ Stützstellen: Quadraturregeln der Ordnung $p=2s$,
%d.h.\ Regeln die Polynome bis zum Grad $2s-1$ exakt integrieren.
%
%\bigskip
%
...
%\end{definition}
%
%\noindent Kollokationsverfahren mit diesen Stützstellen werden Gauß-Verfahren genannt.
%
%\bigskip
%
%Aus unserem Satz~\ref{thm:ordnung_quadratur_kollokation} folgt direkt das folgende Resultat
%zur Konsistenzordnung von Gauß-Verfahren.
%
%\begin{satz}[\cite{deuflhard_bornemann:2008}, Satz~6.43]
...
%\begin{itemize}
%	\item Wir führen einen neuen, stärkeren Stabilitätsbegriff ein, der direkt auf nichtlineare Gleichungen zielt.
%	\item Dann zeigen wir, dass Gauß-Verfahren sogar in diesem stärkeren Sinne stabil sind.
%\end{itemize}
%
...
%
%\begin{definition}
%	Eine Abbildung $f:\Omega_0\to \R^d$ heißt \emph{monoton fallend} oder \emph{dissipativ}
% bzgl.\ eines Skalarproduktes $\langle\cdot,\cdot\rangle$, wenn für alle $x,\bar x\in\Omega_0$
%	\begin{equation*}
...
%\end{definition}
%
%Als nächstes definieren wir eine Variante des Begriffs \glqq stabile Differentialgleichung\grqq.
%\begin{definition}
%	Ein Phasenfluss $\Phi^t:\Omega_0\to\Omega_0$ heißt \emph{nichtexpansiv}, wenn
%	\begin{equation*}
...
%
%\begin{lemma}
%	Sei $x'=f(x)$ Differentialgleichung auf $\Omega_0$ mit lokal Lipschitz-stetigem $f$. Der Phasenfluss $\Phi$ ist genau dann nichtexpansiv, wenn $f$ dissipativ ist.
%\end{lemma}
%\begin{proof}
%	\begin{itemize}
...
%\begin{itemize}
%	\item Sei $f$ dissipativ. Dann ist
%	\begin{equation*}
%	 \chi(t)
%	 =
%	 \chi(0) + \int_0^t 2\underbrace{\langle f(\Phi^sx)-f(\Phi^s\bar x), \Phi^sx-\Phi^s\bar x\rangle}_{\leq 0}\,ds
%	 \leq \chi(0).
%	\end{equation*}
%\end{itemize}
%II) \glqq$\Leftarrow$\grqq\  Sei $\Phi$ nichtexpansiv\\
...
%	Ein Verfahren heißt B-stabil, wenn es für dissipative, hinreichend glatte rechte Seiten einen nichtexpansiven diskreten Phasenfluss erzeugt, also
%	\begin{equation*}
...
%
%\begin{lemma}[\cite{deuflhard_bornemann:2008}, Satz~6.50]
...
%	\end{align*}
%	($\abs{\cdot}$ ist multiplikativ -- komplexer Betrag!)\\
%	Daraus folgt $R(\tau\lambda)\leq 1$ für alle $\tau\geq0$.
%	\begin{itemize}
...
%\begin{satz}[{{\cite{deuflhard_bornemann:2008}}} 6.51]
...
%		\item Wähle $x,\bar x\in\Omega_0$.
%		\item Sofern $\tau$ klein genug ist, existieren die Kollokationspolynome $u,\bar u\in P_s$, mit
%			\begin{equation*}
%				u(0)=x,\ u(\tau)=\Psi^\tau x,
%				\qquad
%				\bar u(0) = \bar x,\ \bar u(\tau) = \Psi^\tau \bar x.
%			\end{equation*}
...
%			\end{equation*}
%		 \item Aber $q(\theta) = \abs{u(\theta\tau)- \bar u(\theta\tau)}^2$ ist ein Polynom in $\theta$
%		  vom Grad höchstens $2s$.
%		 \item Also ist $q'$ ein Polynom vom Grad $2s-1$.
%		 \item Dafür ist Gauß-Quadratur exakt:
%		 	\begin{equation*}
%			 \int_0^1 q'(\theta)\,d\theta = \sum_{j=1}^s b_j q'(c_j).
%			 \end{equation*}
%		 \item Wir zeigen jetzt, dass $q'(c_j)\leq 0\ \forall j=1,\cdots,s$.\\
...
%
%\bigskip
%\todoannot{1.5\baselineskip}{Bild von \{ |R(z)|=1 \}}
%
%\begin{itemize}
...
%	\end{equation*}
%	Dieses Verfahren ist \emph{nicht} B-stabil!
%\end{itemize}
%\textit{Beweisskizze.}
%\begin{itemize}
...
%	\item $x \equiv 0$ ist Fixpunkt der Gleichung und der impliziten Trapezregel.
%	\item Wenn die impl. Tr.Regel B-stabil sein soll, muss also
%	\begin{equation*}
%		\abs{\Psi^\tau x - \underbrace{\Psi^\tau 0}_{=0}} \leq \abs{x-0}
%	\end{equation*}
%	für alle $x\in\R,\ \tau>0$ gelten (Das Verfahren existiert $\forall\tau$.)
%	\item Allerdings erhält man für $x=-2,\ \tau=\frac{36}{7}$ gerade $\Psi^\tau x = 2{,}5$.
%\end{itemize}
...
%\begin{itemize}
%	\item Gauß-Verfahren, es gibt aber noch andere.
%\end{itemize}
...
%
%\begin{definition}
% Ein Zustand $x_*\in\Omega_0$ heißt Fixpunkt der Gleichung, wenn $f(x_*)=0$,
% bzw.\ wenn $\Phi^tx_*=x_*$ für alle $t$ ist.
%\end{definition}
%\begin{definition}
...
%\begin{satz}[{{\cite{deuflhard_bornemann:2008}}} 3.30]
...
%\bigskip
%
%\emph{Zwischenfazit:} Um die asymptotische Stabilität von Fixpunkten zu untersuchen, reicht es, sich die Linearisierung um $ x_* $ anzuschauen!\\
%
...
%\medskip
%
%\emph{Idee}: Wenn $Df(x_*)$ das Stabilitätsverhalten von $x_*$ qualitativ richtig beschreibt,
%dann enthält die \emph{lineare} Gleichung~\eqref{Gleichung:Linearisierte-Differentialgleichung}
%vielleicht schon alle \glqq schwierigen\grqq{} (im Sinne der Stabilität) Aspekte von $ x'=f(x) $
%in der Nähe von $ x_* $?
%
%\medskip
%
...
%\end{itemize}
%\begin{definition}
%	Ein Einschrittverfahren heißt \emph{invariant gegen Linearisierung um einen Fixpunkt $x_*$}, wenn
%	\begin{enumerate}
%		\item $\Psi^\tau x_*=x_*\ \forall\tau>0$ ($\tau$ zulässig)\\
%		$\rightarrow$ der Fixpunkt der Differentialgleichung ist auch
%		 Fixpunkt des numerischen Verfahrens für die nichtlineare Gleichung.
% %
%		\item $\Psi_*^\tau x = x_* + R(\tau Df(x_*))(x-x_*)$ mit einer rationalen Funktion $R$,
%		die nur vom Verfahren abhängt\\
%		$\rightarrow$ D.h.: Für das linearisierte Problem degeneriert das Verfahren zu einer rationalen Approximation der Exponentialfunktion.
% %
%		\item $D_x\Psi^\tau x\vert_{x=x_*}=\Psi^\tau_*$ für alle zulässigen $\tau$\\
%		$\rightarrow$ $\Psi^\tau_*$ ist Linearisierung von $\Psi^\tau$.
%	\end{enumerate}
%\end{definition}
%
%Zum Beispiel sind alle expliziten RK-Verfahren in diesem Sinne invariant. Solch ein Verfahren
%heißt $A$-stabil, falls $R$ $A$-stabil ist.
%
%\medskip
%
%Invariante Verfahren retten den Zusammenhang zwischen der asymptotischen Stabilität
%eines Fixpunkts $x_*$ und der Linearisierung dort ins Diskrete:
%
%\begin{satz}[{{\cite{deuflhard_bornemann:2008}}} 6.23]
% Sei $\Psi^\tau$, $\Psi^\tau_*$ ein gegen Linearisierung invariantes Einschrittverfahren.
%	Sei $\tau_c\geq 0$ die maximale Schrittweite, so dass $\Psi^\tau_*$ die asymptotische Stabilität vererbt. Dann ist $x_*$ asymptotisch stabiler Fixpunkt der Rekursion
%	\begin{equation*}
%	x_{n+1} = \Psi^\tau x_n\qquad n=0,1,2,\hdots
%	\end{equation*}
%	für alle $\tau<\tau_c$.
%\end{satz}
%\underline{Bsp:} Skalare Differentialgleichung
%\begin{equation*}
%x' = \lambda(1-x^2),\qquad \lambda > 0.
%\end{equation*}
%\begin{itemize}
%	\item Fixpunkte:\\
...
%	\end{equation*}
%	\item Explizites Euler-Verfahren dafür
%	\begin{itemize}
%		\item stabil, wenn $\tau< 1/\lambda$
%	\end{itemize}
%	\item Es folgt: $x_s$ ist auch asymptotisch stabiler Fixpunkt des expl. Euler-Verfahrens für die nichtlineare Gleichung
%	\begin{equation*}
...
%	\item Euler: $x_0\in[0, 5/4]$.
%\end{itemize}
%
%\subsection{Linear-implizite Runge--Kutta-Verfahren}
%
%\emph{Idee:} Behandle nur den linearen Teil von $f$ implizit.
%
%\bigskip
%
...
%\begin{itemize}
%	\item Nur ein \emph{lineares} Gleichungssystem pro Schritt
%	\item Trotzdem $A$-stabil
%\end{itemize}
%Jetzt allgemein: Linear-implizite Runge-Kutta-Verfahren
%\begin{equation*}
%\Psi^\tau x = x + \tau \sum_{j=1}^s b_jk_j
%\end{equation*}
%\begin{equation*}
...
%für $i=1,\hdots,s$.\\
%
%\underline{Beachte:} Der obere Summationsindex des impliziten Teils ist $i$, nicht $s$.
%\begin{itemize}
% \item Dadurch kann der Phasenfluss durch wiederholtes Lösen \emph{linearer} Gleichungssysteme berechnet werden.
% \begin{enumerate}
%  \item $J = Df(x)$
%  \item $\displaystyle (I-\tau\beta_{ii}J)k_i = \tau\sum_{j=1}^{i-1}(\beta_{ij}-\alpha_{ij})Jk_j + f\Big(x + \tau\sum_{j=1}^{i-1} \alpha_{ij} k_j\Big)$ für $i=1,\hdots,s$
%  \item $\displaystyle \Psi^\tau x = x + \tau\sum_{j=1}^{s} b_j k_j$
% \end{enumerate}
%\end{itemize}
%Solche Verfahren heißen \emph{lineare-implizite RK-Verfahren} oder \emph{Rosenbrock-Verfahren.}
%
%\medskip
%
%Koeffizienten: $A=(\alpha_{ij})\in\R^{s\times s},\ B=(\beta_{ij})\in\R^{s\times s},\  b=(b_1\hdots,b_s)$
%\begin{itemize}
%	\item Wählt man die $\beta_{ij}$ alle gleich, so haben die $s$ Gleichungssysteme in 2) alle die gleiche Matrix\\
...
%\end{lemma}
%\begin{proof}
%	Zu zeigen: Unter den gegebenen Voraussetzungen hat $I-\tau\beta J$ nicht den Eigenwert 0.
%	\begin{itemize}
%		\item Nach Satz \eqref{thm:spektrale_aequivarianz} über rationale Funktionen ist aber
%		\begin{equation*}
...
%	\begin{equation*}
%	\Re(1-\tau\beta\lambda) = 1-\tau\beta \Re(\lambda) \geq 1\ \Rightarrow\ 1-\tau\beta\lambda \neq 0.
%	\end{equation*}
%	\underline{Fall 2:} $0<\Re(\lambda)\leq \nu(J)$:\\
%	\begin{equation*}
%	\Re(1-\tau\beta\lambda) = 1 - \tau \beta \Re(\lambda) \geq 1 - \tau\beta\nu(J).
%	\end{equation*}
%	Also $>0$ wenn $\tau < \frac{1}{\beta\nu(J)}$.
%\end{proof}
%Der Satz sagt: Die steifen Anteile der Differentialgleichung (d.h., die nichtpositiven Eigenwert von $J$),
%beeinflussen nicht Lösbarkeit des Gleichungssystems.
%
%\medskip
%
%Für autonome \emph{lineare} Probleme ist das Verfahren offensichtlich äquivalent zum impliziten
%Runge-Kutta-Verfahren $(b, (\beta_{ij}))$.  Es hat also die selbe Stabilitätsfunktion.
%
%\medskip
%
