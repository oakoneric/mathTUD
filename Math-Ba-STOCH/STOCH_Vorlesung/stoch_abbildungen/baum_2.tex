...
\begin{document}
	...
	\begin{tikzpicture}[level distance=1.5cm,
		...
		child {node {$W$}
			...
			child {node {$R^\complement$} edge from parent node[right]  {$0$}}
			edge from parent
			node[left]  {$\sfrac{2}{3}$}
		}
		child {node {$W^\complement$}
			child {node {$R$} edge from parent node[left]  {$\sfrac{1}{4}$}}
			child {node {$R^\complement$} edge from parent node[right]  {$\sfrac{3}{4}$}}
			edge from parent
		...
		child {node {$R$}
			...
			child {node {$W^\complement$} edge from parent node[right]  {$\sfrac{1}{9}$}}
			edge from parent
			node[left]  {$\sfrac{3}{4}$}
		}
		child {node {$R^\complement$}
			child {node {$W$} edge from parent node[left]  {$0$}}
			child {node {$W^\complement$} edge from parent node[right]  {$1$}}
			edge from parent
