...
import sys
from makeunreadable import (replaceArgumentLessCommand, findNextCommand,
                            findNextBracket)
    ...
    with open(textfilename, mode="r") as textfile:
        ...
        for (myCommand, myReplacement) in toBeReplacedWithoutArgument:
            textext = replaceArgumentLessCommand(textext, myCommand,
                                                 myReplacement)
        for old, new in getReplacePairs():
            textext = textext.replace(old, new)
        texlines = textext.splitlines(keepends=True)
        for lineNr in range(len(texlines)):
            for opening, closing, maxArgLen, newOpening, newClosing in bracketPairs():
                texlines[lineNr] = replaceAllBracketPairs(
                    texlines[lineNr], opening, closing, maxArgLen,
                    newOpening, newClosing)
        toBeWarned = warningTriggeringCommands()
        for command in toBeWarned:
            texlines = insertWarnings(texlines, command)
